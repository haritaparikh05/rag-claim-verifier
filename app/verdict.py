...
A passage only counts as evidence if it actually addresses the same specific attribute as the claim - not merely because it shares a keyword. For example, a safety warning about not letting a power cord touch a stovetop says nothing about whether the appliance itself is compatible with induction cooktops as a cooking surface; sharing the word "stovetop" does not make it relevant evidence. Likewise, a restriction on which surfaces an appliance's removable accessories (like a lid or inner pot) may be placed on is not evidence about whether the appliance's own base unit works on a given cooktop type - those are different subjects, even when the passage lists specific cooktop types (ceramic, electric coil, gas) and the claim is about a type not on that list (e.g. induction). If the passages don't genuinely speak to the claim's actual subject, the verdict is "unverifiable", not "contradicted".
